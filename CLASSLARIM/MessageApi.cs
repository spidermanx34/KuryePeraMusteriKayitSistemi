...
namespace KuryePera.CLASSLARIM
{
    internal class MessageApi
    {
        ...
        public void SmsSender(string TelNo, string SmsText, string SmsText2)
        {
            ...
            testXml += "<username>5466962140</username>";// ileti merkezi kullanııc adınız
            testXml += "<password>72687268</password>";// ileti merkezi kullanıcı şifreniz
            testXml += "</authentication>";
            testXml += "<order>";
            testXml += "<sender>APITEST</sender>";// Size ait olan sms başlığınız
            testXml += "<sendDateTime></sendDateTime>";
            testXml += "<message>";
            testXml += $"<text>{SmsText},{SmsText2}</text>";
            testXml += "<receipents>";
